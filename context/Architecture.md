...
> These are hard constraints for any AI coding agent working on this codebase.

### Security Rules

1. **NEVER store plain-text passwords.** Always use bcrypt hashing with salt.
2. **NEVER expose JWT secrets, database credentials, or API keys in source code.** Use environment variables exclusively.
3. **NEVER return password hashes in API responses.** Exclude from all Pydantic response schemas.
4. **NEVER trust client-side input without validation.** All inputs must be validated through Pydantic schemas.
5. **NEVER disable CORS in production.** Always configure allowed origins explicitly.

### Data & Privacy Rules

6. **NEVER store raw audio files permanently.** Process, extract features, then delete. Feature-only logging by default.
7. **NEVER log personally identifiable information (PII)** such as phone numbers, names, or email addresses in application logs.
8. **NEVER transmit raw audio over unencrypted channels.** All audio data must flow over HTTPS/WSS.
9. **NEVER retain analysis data beyond the configured retention period.**

### Architecture Rules

10. **NEVER call the ML service directly from the frontend.** All ML requests must go through the Backend API.
11. **NEVER put business logic in API route handlers.** Routes call services; services contain logic.
12. **NEVER use raw SQL queries.** Use SQLAlchemy ORM exclusively.
13. **NEVER hardcode configuration values** (ports, URLs, thresholds). Use environment variables or the configurations table.
14. **NEVER create circular imports.** Follow the dependency direction: routers → services → models/schemas.

### Code Quality Rules

15. **NEVER commit without type hints** on function signatures (Python) and prop types (TypeScript).
16. **NEVER suppress errors silently.** All exceptions must be caught, logged, and handled with appropriate HTTP status codes.
17. **NEVER use `any` type in TypeScript** unless absolutely unavoidable and documented with a comment.
18. **NEVER duplicate code across services.** Extract shared logic into utility modules.

### Skill & Workflow Rules

19. **ALWAYS run `/architect` before complex features** — ML pipeline, WebSocket, analysis pipeline, real-time dashboard. Think before building.
20. **ALWAYS run `/imprint` after building any UI component.** Then update `context/UI-registry.md`.
21. **ALWAYS update `context/Progress-tracker.md`** after completing any feature.
22. **ALWAYS load a library's installed skill** (from `C:\Users\ACER\.agents\skills\`) before using it. Then read `context/Library-docs.md`.
23. **ALWAYS read `node_modules/next/dist/docs/`** before writing Next.js code. APIs may differ from training data.
24. **NEVER attempt the same fix more than once.** If a bug persists after one corrective prompt, run `/recover` immediately.
25. **ALWAYS run `/remember save`** at the end of a session if a feature is incomplete, and `/remember restore` at the start of the next session.
26. **ALWAYS run `/review`** before Phase 4 (demo prep) and before the final demo.
